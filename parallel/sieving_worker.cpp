...
namespace quadratic_sieve{
	...
	int *info=new int[2]; // contains start and end for the sieving process
	...
	int broadcast_phase=0;  /* 0 - info not broadcasted, 1 - info is being broadcasted(recieved)
														 * ***/

	std::queue<b_data> q_b_data;
	std::queue<b_info> q_i_data;

	MPI_Request req_broa;  // broadcast request(both info and data 
	...
	void sieving_work(){
		...
		while (!all_sieving_recieved()){
			...
			if ( flag == true ) { // we have something to do
				if (phase == 0 ) {
					...
					MPI_Test(&req,&flag,&status);
					if ( flag == true){
						prepare_broadcast(matrix_index);
						int dummy=0;
		...
		for ( std::vector<b_data>::iterator it=q_b_data.begin;it!=q_b_data.end();it++)
			free [] (it->data);
		}
	}


	/*** try to broadcast data ***/
	void try_broadcast_step(){
		int flag; MPI_Status status;
		if ( broadcast_phase == 0 ){
			if ( q_i_data.empty()) return;  // no data to transfer!
			b_info b_i=q_i_data.front();
			q_i_data.pop();	
			
			// send messages
			//MPI_Ibcast(b_i.info,4,MPI_INT,mpi_rank,MPI_COMM_WORLD,&req_broa);
			//printf("BROADCASTING FROM %d\n",mpi_rank);
			MPI_Request tmp_req;
			for ( int i=0;i<mpi_size;i++){
				if ( i == mpi_rank ) continue;
				MPI_Isend(&dumm,1,MPI_INT,mpi_rank,MPI_COMM_WORLD,&tmp_req);
			}
			broadcast_phase=0;
		} else if (broadcast_phase == 1){
			MPI_Test(&req_broa,&flag,&status);
			if ( flag == true ){
				//b_data b=q_b_data.front();
				//q_b_data.pop();
				//MPI_Ibcast(b.data,b.sz,MPI_INT,mpi_rank,MPI_COMM_WORLD,&req);
				//broadcast_phase=2;
			}
		} else if ( broadcast_phase == 2 ){
			MPI_Test(&req,&flag,&status);
			if ( flag == true ){
				/*** finalize memory consumption ***/
				b_data b=q_b_data.front();
				q_b_data.pop();
				delete []b.data;
				broadcast_phase=0;
	...
	void prepare_broadcast(int matrix_index){
		b_data b;
		b_info b_i;
		...
		b_i.info[1]=b_i.info[0];
		b_i.info[0]+=found_exponents_binary.size()-1;
		b_i.info[2]=found_smooth_numbers.size();
		b_i.info[3]=matrix_index;
		q_i_data.push(b_i);
		
		sz=0; for ( int i=0;i<3;i++) sz+=b_i.info[i];
		b.data=new int[sz];
		int index=0;
		for ( unsigned int i=0;i<found_exponents_binary.size();i++){
			if (i!=0 ) 
		...
		for ( unsigned int i=0;i<found_exponents_binary.size();i++){
			std::vector<int> dat=found_exponents_binary[i].getData();
			for ( std::vector<int> ::iterator it=dat.begin();it!=dat.end();it++){
				b.data[index++]=found_exponents[i](*it);
			}
		}
	...
	void sieve_interval(int start, int end){
		...
		for ( int cnt=0;cnt<2;cnt++){ // taking care of both signs
			for ( int i=0;i<sz;i++){
